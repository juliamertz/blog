...
function getArgumentLists(funcs) {
  const result = [];
  for (const shape of funcs) {
    const args = getArguments(shape.path);
    result.push(args);
  }
  return result;
}
function pts(sideCount, radius) {
  ...
  return vertexIndices.map((index2) => {
    return {
      theta: offset + degreesToRadians(angle * index2),
      r: radius
    };
  });
}
...
function polygon([cx, cy], sideCount, radius) {
  return pts(sideCount, radius).map(({ r: r2, theta }) => [cx + r2 * Math.cos(theta), cy + r2 * Math.sin(theta)]).join(" ");
}
function generatePolygon([cx, cy], sideCount, radius) {
  const res = polygon([cx, cy], sideCount, radius);
  return res;
}
function getPathGenerator(size, offset) {
  const S2 = size;
  const O2 = offset;
  const shapes = [
    // no. 1
    ...
    // undordered paths from here on
    // no. 7
    {
      type: "path",
      args: [295, 120, 300, 100, 100],
      rulers: [
        {
          key: "H",
          position: "left",
    ...
    },
    {
      type: "polygon",
      args: [295, 295, 8, 295],
      path: (cx, cy, sideCount, radius) => {
        const path = generatePolygon([cx, cy], sideCount, radius);
        return path;
      }
    }
  ];
  const argumentsList = getArgumentLists(shapes);
  const getDefaultArgs = (shape) => {
    const defaultSize = 300;
    const args = shapes[shape].args;
    return args.map((arg) => arg / defaultSize * size);
  };
  return {
    shapes,
    argumentsList,
...
const getRulers = (shape, args, generator, size, offset) => {
  const shapes = generator.shapes;
  const argumentsList = generator.argumentsList;
  const S2 = size;
  const O2 = offset;
  const RO = O2 + 15;
  const rulers = shapes[shape].rulers;
  const result = {
    top: [],
    bottom: [],
    left: [],
  ...
  rulers == null ? void 0 : rulers.forEach((ruler) => {
    const argumentIndex = argumentsList[shape].indexOf(ruler.key);
    const length = ruler.flip ? S2 - args[argumentIndex] : args[argumentIndex];
    const rulerTemplates = {
      top: `M ${ruler.start} ${O2 * 3 + RO} V ${O2 + RO} H ${length} V ${O2 * 3 + RO}`,
      bottom: `M ${ruler.start} ${O2} V ${O2 * 3} H ${length} V ${O2}`,
      left: `M ${O2 * 3 + RO} ${ruler.start} H ${O2 + RO} V ${length} H ${O2 * 3 + RO}`,
      right: `M ${O2} ${ruler.start} H ${O2 * 3} V ${length} H ${O2}`
    };
    const path = rulerTemplates[ruler.position];
    result[ruler.position].push({
      ...ruler,
      path,
      length: args[argumentIndex]
    });
  });
  return result;
};
function GlasVorm({
  ...
  const generator = getPathGenerator(size, strokeWidth);
  const shape = generator.shapes[selectedShape];
  const path = shape["path"](...args).split("\n").map((line) => line.trim()).join(" ");
  const rulers = getRulers(selectedShape, args, generator, size, strokeWidth);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "shape-wrapper", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { width: size + strokeWidth, height: size + strokeWidth, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "path",
          {
            fill: "transparent",
            strokeWidth,
...
function Rulers({ orientation, rulers: paths, size, position }) {
  const isHorizontal = orientation === "horizontal";
  const width = 40;
  const getHorizontalPosition = (ruler) => {
    const center = (ruler.length + 5) / 2;
    return center;
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      style: {
        ...
        alignSelf: position === "top" ? "end" : "",
        justifySelf: position === "left" ? "end" : ""
      },
      children: /* @__PURE__ */ jsxRuntimeExports.jsx(
        "svg",
        {
          width: isHorizontal ? size : `${width}px`,
          height: isHorizontal ? `${width}px` : size,
          children: paths.map((path, index2) => /* @__PURE__ */ jsxRuntimeExports.jsxs(reactExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "text",
              {
                x: orientation === "vertical" ? "20px" : getHorizontalPosition(path),
                y: orientation === "horizontal" ? "20px" : "50%",
                textAnchor: "middle",
                color: "red",
                children: path.key
              }
...
function ShapeSelector({
  ...
  const [_, setSelectedShape] = state;
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: Object.keys(shapes).map((shape) => /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      style: { marginRight: "3px" },
      onClick: () => setSelectedShape(parseInt(shape)),
      children: parseInt(shape) + 1
    },
    shape
  )) });
}
function ArgsInput({
  ...
  const [args, setArgs] = state;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
    requiredArgs.map((arg, index2) => {
      const changeHandler = (e) => {
        const value = e.target.value;
        const newArgs = [...args];
        newArgs[index2] = parseInt(value);
        setArgs(newArgs);
      };
      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          style: { display: "grid", gridTemplateColumns: "3ch 1fr" },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { width: "3ch" }, children: arg }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                placeholder: arg,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        onClick: () => navigator.clipboard.writeText(JSON.stringify(args)),
        children: "Copy values to clipboard"
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        ...
        onClick: () => {
          const svgWrapper = document.getElementById("shape-wrapper");
          if (!svgWrapper)
            return;
          const svg = svgWrapper.innerHTML;
          navigator.clipboard.writeText(svg);
        },
        children: "Copy svg to clipboard"
      }
    )
  ] });
}
function App() {
  ...
  const SIZE = 300;
  const generator = getPathGenerator(SIZE, STROKE_WIDTH);
  const [selectedShape, setSelectedShape] = reactExports.useState(0);
  const [args, setArgs] = reactExports.useState(
    generator.getDefaultArgs(selectedShape)
  );
  reactExports.useEffect(() => {
    setArgs(generator.getDefaultArgs(selectedShape));
  }, [selectedShape]);
  const requiredArgs = generator.argumentsList[selectedShape];
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(ArgsInput, { requiredArgs, state: [args, setArgs] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      ShapeSelector,
      {
        state: [selectedShape, setSelectedShape],
